...
export default async function handler(req: Request) {
    ...
    await admin.from("tasks").update({ status: "paused" }).eq("id", task_id).eq("user_id", user_id);
    await admin.from("focus_sessions").update({ ended_at: new Date().toISOString(), outcome: "paused" })
